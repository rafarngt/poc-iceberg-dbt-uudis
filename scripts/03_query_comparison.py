...
spark.sql("""
    SELECT
        ...
        status,
        _silver_updated_at
    FROM local.silver.silver_cost_centers_scd1
    ORDER BY center_cd
""").show(truncate=False)
...
spark.sql("""
    SELECT
        ...
        valid_from_sk,
        is_current
    FROM local.silver.silver_cost_centers_scd2
...
spark.sql("""
    SELECT
        ...
        has_history,
        scd2_total_versions
    FROM local.gold.gold_cost_centers_comparison
    ORDER BY center_cd
""").show(truncate=False)
...
spark.sql("""
    ...
    FROM local.bronze.bronze_cost_centers.snapshots
